...
Here's a claim, and we'll back every word of it: **you can fully index, serve, and query Uniswap V3
— or any other web3 protocol, for that matter — on your own machine, for free, in under an hour,
...
- **Bare public URLs work but wobble.** Some serve `getLogs` fine; many rate-limit, 403, or cap block
  ranges. nuthatch fails over across endpoints and retries transient blips, so it rides out the wobble
  — but a keyed endpoint is calmer.
